...
//    @IBInspectable var radius: CGFloat = 290 {
//        didSet {
//            setNeedsDisplay()
//        }
//    }
...
//                                       height: radius * 2))
//
//        let ellipseLayer = CALayer()
//        ellipseLayer.draw(in: context)
        ...
        let imageBiggerSide = myLayer.frame.size.height > myLayer.frame.size.width ? myLayer.frame.size.height : myLayer.frame.size.width
        
        let circleLayer = CAShapeLayer()
        let radius: CGFloat = imageBiggerSide as CGFloat / 2
        circleLayer.path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: 2.0 * radius, height: 2.0 * radius), cornerRadius: radius).cgPath
        circleLayer.position = CGPoint(x: self.frame.midX - radius, y: self.frame.midX - radius)
//        myLayer.frame = self.bounds

        circleLayer.fillColor = UIColor.blue.cgColor
        circleLayer.shadowColor = UIColor.black.cgColor
        circleLayer.shadowOpacity = 0.8
        circleLayer.shadowRadius = 18
        circleLayer.shadowOffset = CGSize.zero
        self.layer.addSublayer(circleLayer)
        
        
                myLayer.position = CGPoint(x: self.frame.midX - radius, y: self.frame.midX - radius)
        myLayer.contents = myImage
        myLayer.cornerRadius = myLayer.frame.size.height / 2;
        myLayer.masksToBounds = true;
        myLayer.borderWidth = 0;
...
class TestController: UIViewController {
    ...
    // MARK: - Outlets
    @IBOutlet var sbTestView: UIView!
...
//        sbTestView = TestView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
...
////        sbTestView.layer.addSublayer(gradientLayer)
